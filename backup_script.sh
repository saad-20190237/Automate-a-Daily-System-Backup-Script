...
EMAIL="saad.ahmed.200168@gmail.com"

mkdir -p "/home/vagrant/backups" "/home/vagrant/logs"

# Validate BACKUP_SOURCE
if [ -z "$BACKUP_SOURCE" ]; then
    echo "Error: No backup source specified." | mail -s "Backup Error: Missing Argument" "$EMAIL"
    exit 1
fi

# Perform backup with error handling
if ! tar -czf "${BACKUP_DEST}" "${BACKUP_SOURCE}" 2>> "$LOG_FILE" ; then
    echo "There was an error in archiving or compression" | mail -s "Backup Error" "$EMAIL"
    exit 1
fi
